        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form method="POST" action="<?php echo url_for(array('sf_route' => 'urgence-urgence-update')) ?>" id="formEdit" class="form-horizontal">
                        <input type="hidden" name="idUrgence" value="<?php echo $urgence->getIdUrgence() ?>"/> 

                        <div class="form-group" style="<?php echo ( isset($urgence) && $urgence->getUrgent() ? '' : 'display: none;') ?>">
                            <label class="col-sm-2 control-label">
                                <?php echo __('Fourchette date de livraison') ?>
                            </label>
                            <div class="col-sm-10">
                                <input <?php echo $editUrgence ? "" : "disabled=''" ?> type="text" id="date_livraison_debut" name="date_livraison_debut" class="form-control dateHeure"  value="<?php echo isset($urgence) && $urgence->getDateLivraisonDebut() ? DateTime::createFromFormat('Y-m-d H:i:s', $urgence->getDateLivraisonDebut())->format('d/m/Y H:i:s') : ''; ?>" />
                                <input <?php echo $editUrgence ? "" : "disabled=''" ?>  type="text" id="date_livraison_fin" name="date_livraison_fin" class="form-control dateHeure" value="<?php echo isset($urgence) && $urgence->getDateLivraisonFin() ? DateTime::createFromFormat('Y-m-d H:i:s', $urgence->getDateLivraisonFin())->format('d/m/Y H:i:s') : ''; ?>" />
                            </div>
                        </div>
                        <div class="form-group" style="<?php echo ( isset($urgence) && $urgence->getUrgent() ? '' : 'display: none;') ?>">
                            <label class="col-sm-2 control-label">
                            ...
                            <div class="col-sm-10">
                                <select <?php echo $editUrgence ? "" : "disabled=''" ?>  id="contact_pff" class="chosen-select" name="contact_pff">
                                    <option value="-1" <?php echo (!isset($urgence) ? 'selected' : '') ?>>N/C</option>
